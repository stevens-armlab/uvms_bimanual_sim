...
class PositionControllerNode:
    def __init__(self):
        ...
        # self.pid_rot_x = PIDRegulator(1, 0, 0, 1)
        # self.pid_rot_y = PIDRegulator(1, 0, 0, 1)
        # self.pid_rot_z = PIDRegulator(1, 0, 0, 1)
        ...
        self.pub_cmd_vel = rospy.Publisher('cmd_vel_ang', geometry_msgs.Twist, queue_size=10)
    ...
    def odometry_callback(self, msg):
        ...
        # vx_angular = self.pid_rot_x.regulate(e_rot[0], t)
        # vy_angular = self.pid_rot_y.regulate(e_rot[1], t)
        # vz_angular = self.pid_rot_z.regulate(e_rot[2], t)
        # v_angular = numpy.array([vx_angular, vy_angular, 0])
    ...
    def config_callback(self, config, level):
        ...
        # self.pid_rot_x = PIDRegulator(config['rot_p'], config['rot_i'], config['rot_d'], config['rot_sat'])
        # self.pid_rot_y = PIDRegulator(config['rot_p'], config['rot_i'], config['rot_d'], config['rot_sat'])
        # self.pid_rot_z = PIDRegulator(config['rot_p'], config['rot_i'], config['rot_d'], config['rot_sat'])
